    ...
    D: Clone + Debug + Default + DeserializeOwned + PartialEq + Serialize,
{
    ...
    /// Apply `f` to the cached value in place **without persisting it**. On a
    /// cold cache the backing object is read once first, so a process that has
    /// never seen this key (e.g. a produce arriving after restart before any
    /// `InitProducerId` reseeded it here) validates against the durable state
    /// rather than an empty default; thereafter the in-memory value is the
    /// fast-path authority and is persisted lazily via [`Self::checkpoint`].
    ///
    /// This is the read/advance half of the debounced idempotent-sequence path
    /// (#48): the per-batch `producers/{id}.json` PUT that [`Self::with_mut`]
    /// would issue is deferred, cutting an all-idempotent produce workload from
    /// two S3 PUTs per batch to ~one.
    #[instrument(skip_all, fields(path = %self.path))]
    pub(super) async fn mutate_cached<E, F>(
        &self,
    ...
    where
        F: Fn(&mut D) -> Result<E>,
    {
        REQUESTS.add(1, &[KeyValue::new("method", "mutate_cached")]);

        if self.data_version.lock().map(|guard| guard.is_none())? {
            self.get(object_store).await?;
        }
...
mod tests {
    ...
    /// max-merge reconcile used by the lazy producer checkpoint (#48).
    fn max_merge(dst: &mut X, src: &X) {
        dst.0 = dst.0.max(src.0);
    }

    #[tokio::test]
    async fn mutate_cached_defers_persistence() -> Result<()> {
        let _guard = init_tracing()?;

        let path = Path::from("/abc/deferred.json");
        let object_store = InMemory::new();
        let o = OptiCon::path(path.clone());

        // Several in-memory advances, and yet no object is written.
        for _ in 0..5 {
    ...
    #[tokio::test]
    async fn mutate_cached_warms_from_durable_object() -> Result<()> {
        let _guard = init_tracing()?;

        let path = Path::from("/abc/warm.json");
        let object_store = InMemory::new();

        // A prior process persisted X(10).
        ...
        // A fresh handle (cold cache) must validate against the durable value,
        // not an empty default — the restart-dedup guarantee (#48).
        let o = OptiCon::<X>::path(path.clone());
        let seen = o.mutate_cached(&object_store, |x: &mut X| Ok(x.0)).await?;
        assert_eq!(10, seen);
    ...
    async fn checkpoint_merges_concurrent_write() -> Result<()> {
        ...
        let path = Path::from("/abc/merge.json");
        let object_store = InMemory::new();

        // Our handle accumulates X(3) in memory (never persisted -> Create).
        let ours = OptiCon::path(path.clone());
        ours.mutate_cached(&object_store, |x: &mut X| {
            x.0 = 3;
            Ok(())
        })
        .await?;

        // A concurrent writer creates the object at X(5) first.
        OptiCon::path(path.clone())
            .with_mut(&object_store, |x: &mut X| {
                x.0 = 5;
                Ok(())
